...
#pragma endregion Contains the VTable Indexs
	...
	//g_pViewRenderBeams = *reinterpret_cast< IViewRenderBeams** >(GameUtils::FindPattern1(strenc("client.dll"), strenc("A1 ? ? ? ? 56 8B F1 B9 ? ? ? ? FF 50 08")) + 0x1);

	//g_pViewRender = **reinterpret_cast< IViewRender*** > (U::FindSig(strenc("client.dll"), strenc("FF 50 14 E8 ?? ?? ?? ?? 5D")) - 0x7); // class not yet made
