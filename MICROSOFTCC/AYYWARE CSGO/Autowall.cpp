...
bool CanWallbang(float &dmg)
{
	...
	CBaseCombatWeapon* weapon = (CBaseCombatWeapon*)Interfaces::EntList->GetClientEntityFromHandle((HANDLE)local->GetWeaponHandle());
...
void DrawAutowallCrosshair()
{
	...
	char nameBuffer[512];
	sprintf_s(nameBuffer, "%d", damage);

	if (CanWallbang(damage))
	{
		Render::DrawOutlinedRect(MidX - 2, MidY - 2, 5, 5, Color(0, 255, 0));
		RECT text = Render::GetTextSize(Render::Fonts::Default, nameBuffer);
		Render::Text(MidX - (text.right / 2), MidY - 18, Color(255, 255, 255, 210), Render::Fonts::Default, nameBuffer);
	}
	else
	{
		Render::DrawOutlinedRect(MidX - 2, MidY - 2, 5, 5, Color(255, 0, 0));
	}
}